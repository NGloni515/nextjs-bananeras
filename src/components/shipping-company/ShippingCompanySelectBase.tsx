...
const chakraStyles: ChakraStylesConfig<Partial<ShippingCompanyType>, false, GroupBase<Partial<ShippingCompanyType>>> = {
...
const shippingCompanyComponents = {
    DropdownIndicator: (
        props: DropdownIndicatorProps<Partial<ShippingCompanyType>, false, GroupBase<Partial<ShippingCompanyType>>>
    ): JSX.Element => (
...
const ShippingCompanySelectBase: React.FC<ShippingCompanySelectBaseProps> = ({ name, placeholder, onChange }) => {
    const { paginationParams } = usePagination();
    const { data = [], isLoading, error } = useShippingCompanies({
        ...paginationParams,
    });
    ...
    const handleChange = (newValue: SingleValue<Partial<ShippingCompanyType>>): void => {
        onChange(newValue);
    };
    ...
    return (
        <ChakraSelect
            ...
            noOptionsMessage={() => error ? 'Error al cargar navieras' : 'No hay navieras disponibles'}
            isLoading={isLoading}
            options={data}
            getOptionLabel={(opt: Partial<ShippingCompanyType>) => opt.name || ''}
            getOptionValue={(opt: Partial<ShippingCompanyType>) => opt.id ? opt.id.toString() : ''}
